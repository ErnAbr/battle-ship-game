...
router.get("/verify/:token", async (req, res) => {
  try {
    ...
    if (!user) {
      return res.redirect(`${process.env.ORIGIN}/verify-email?status=failed`);
    }
    ...
    return res.redirect(`${process.env.ORIGIN}/verify-email?status=success`);
  } catch (error) {
    console.error(error);
    return res.redirect(`${process.env.ORIGIN}/verify-email?status=error`);
  }
});
